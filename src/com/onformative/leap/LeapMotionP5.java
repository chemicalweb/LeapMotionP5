...
public class LeapMotionP5 {
	...
	/**
	 * this class gives you some high level access to the data tracked and
	 * recorded by the leap. it gives you a different way of access than the
	 * original leap sdk and transforms all data into processing equivalent
	 * datatypes.
	 * 
	 * @param PApplet the processing applet
	 */
	public LeapMotionP5(PApplet p) {
		PApplet.println("Starting Leap Motion SDK " + getSDKVersion());
	...
	/**
	 * this prints out the current offset of the vectors from the sdk. this is
	 * just for information and will give you the position, velocity and
   ...
   */
	...
	/**
	 * this returns a pvector containing the velocity offset. the problems the
	 * velocity offset has with it, is that the velocity is slightly shiftet.
	 * for example if you shouldnt have any velocity, because the finger is
	 * stanging still its returning a velocity that is initialized with a new
	 * Finger object. this should be fixed with the upcoming sdk releases (i
	 * hope) *
	 * 
	...
	/**
	 * this retuns an pvector containing the acceleration offset, that has to be
	 * substracted from every vector returned from the library. unfortunately
	 * the leap sdk has a little bug there.
	 * 
	...
	/**
	 * returns the most current frame from the leap sdk. a frame contains every
	 * tracked data from the leap about your fingers.
	 * 
	...
	public Frame getFrame() {
		try {
			...
			System.err
					.println("Can not return current frame. Returning new Frame object instead");
	 ...
	 * 
	 * @param id
	 *            the id of the frame you want
	 * @return Frame the frame which id you passed as a parameter. if the frame
	 *         with the id you asked for is not currently saved anymore you'll
	...
	public Frame getLastFrame() {
		return getFrames().size() >= 2 ? getFrames().get(getFrames().size() - 2) : null;
	}
	 ...
	 * @return the image
	 */
	public PImage getLeftImage() { 
		return leapImage2PImage(currentLeftImage);
	}

	public PImage leapImage2PImage(Image img) {
		if(img == null) return null;
		
		PImage pimg = p.createImage(img.width(), img.height(), PApplet.RGB);
		pimg.loadPixels();

		// Get byte array containing the image data from Image object
		byte[] imageData = img.data();

		// Copy image data into display object, in this case PImage
	...
	/**
	 * returns a CopyOnWriteArrayList<Frame> containing all recently buffered
	 * frames.
	 * 
	...
	public CopyOnWriteArrayList<Frame> getFrames() {
		try {
			...
			System.err
					.println("Can not return list of last frames. Returning empty list instead.");
			System.err.println(e);
	...
	public PVector getFingerPositionXYPlane() {
		...
		if (frame.hands().isEmpty() == false) {
			...
			if (hand.fingers().isEmpty() == false) {
				...
				fingerPositionXYPlane.x = transformLeapToScreenX(finger
						.tipPosition().getX());
				fingerPositionXYPlane.y = transformLeapToScreenY(finger
						.tipPosition().getY());
			}
		}
	...
	/**
	 * converts the x coordinate from the leap space into the processing window
	 * space
	 * 
	 * @param x
	 *            leap-space
	...
	public float transformLeapToScreenX(float x) {
		/*
		 * int startX = -243; int endX = 256; float valueMapped = PApplet.map(x,
		 * startX, endX, 0, p.width); return valueMapped;
		 */
	...
	/**
	 * converts the y coordinate from the leap space into the processing window
	 * space
	 * 
	 * @param y
	 *            leap space
	...
	public float transformLeapToScreenY(float y) {
		/*
		 * int startY = 50; int endY = 350; float valueMapped = PApplet.map(y,
		 * startY, endY, 0, p.height); return valueMapped;
		 */
		return PApplet.lerp(this.p.height, 0.0f, y / LEAP_HEIGHT);
	}

	/**
	 * converts the z coordinate from the leap space into the processing window
	 * space
	 * 
	 * @param z
	 *            leap space
	...
	public float transformLeapToScreenZ(float z) {
		/*
		 * int startZ = -51; int endZ = 149; float valueMapped = PApplet.map(z,
		 * startZ, endZ, 0, p.width); return valueMapped;
		 */
	 ...
	 * 
	 * @param vector
	 *            from the leap sdk containing a position in the leap space
	 * @return the vector in PVector data type containing the same position in
	 *         processing window space
	 */
	public PVector vectorToPVector(Vector vector) {
		return convertLeapToScreenDimension(vector.getX(), vector.getY(),
				vector.getZ());
	}

	/**
	 * converts x, y and z coordinates of the leap to the dimensions of your
	 * sketch
	 * 
	 * @param x
	 *            x position in leap world coordinate system
	 * @param y
	 *            y position in leap world coordinate system
	 * @param z
	 *            z position in leap world coordinate system
	 * @return PVector the pvector of the point you passed in converted to the
	 *         dimensions of your processing sketch window
	 */
	 ...
	 * 
	 * @return ArrayList<Hand> an arraylist containing all currently tracked
	 *         hands
	 */
	...
	/**
	 * gets a hand by number. the number is indicated by the order the hand
	 * appeared in the leap. so the first hand tracked has the nr 0 and the
	 * second one the number 1. once one hand of them two leaves the leap the
	 * one hand left has the nr 0. this is implemented like that because the
	 * leap is loosing track of the id's of hand to easily.
	 * 
	...
	/**
	 * returns averaged translation of all points tracked by the leap in
	 * comparison to the last frame
	 * 
	 * @return
	 */
	public PVector getTranslation() {
		PVector translation = vectorToPVector(getFrame().translation(
				getLastFrame()));
	...
	/**
	 * returns averaged translation of all points tracked by the leap in
	 * comparison to the frame you passed in the method
	 * 
	 * @return
	 */
	...
	public float getPitch(Hand hand) {
		// return PApplet.map((float) Math.toDegrees(hand.direction().pitch()),
		// 0, 22, 0,
		// PConstants.TWO_PI);
	...
	public float getRoll(Hand hand) {
		// return -PApplet.map((float) Math.toDegrees(hand.direction().roll()),
		// 0, 180, 0,
		// PConstants.TWO_PI);
	...
	public Finger getFinger(int fingerNr) {
		...
		if (getFingerList().size() > fingerNr) {
			this.lastDetectedFinger
					.put(fingerNr, getFingerList().get(fingerNr));
		}
	...
	public PVector getTip(Pointable pointable) {
		return convertLeapToScreenDimension(pointable.tipPosition().getX(),
				pointable.tipPosition().getY(), pointable.tipPosition().getZ());
	}

	/**
	 * sets the current screen for gettings the calibrated points. I should
	 * rewrite this, but nobody is gonna read it anyway. arr.
	 * 
	 * @param screenNr
	 */
	...
	public PVector getVelocityOnScreen(Pointable pointable) {
		...
		try {
			oldLoc = getLastController()
					.locatedScreens()
					.get(this.activeScreenNr)
					.intersect(
							getPointableById(pointable.id(), getLastFrame()),
							true);
			loc = this.controller.locatedScreens().get(this.activeScreenNr)
					.intersect(pointable, true);
	...
	/**
	 * returns the origin of the pointable. the origin is the place where the
	 * pointable leaves the body of the hand.
	 * 
	 * @param pointable
	 *            the pointable you want the origin of
	 * @return a PVector containing the position of the origin of the passed
	 *         pointable
	 */
	public PVector getOrigin(Pointable pointable) {
		...
		anklePos = new Vector(pointable.tipPosition().getX() - direction.x,
				pointable.tipPosition().getY() - direction.y, pointable
						.tipPosition().getZ() - direction.z);
	 ...
	 * 
	 * @param pointable
	 *            the pointable you want the velocity of
	 ...
	 * 
	 * @param pointable
	 *            the pointable you want the direction of
	...
	/**
	 * calculates the acceleration of the pointable according to the velocity of
	 * the curent and the last frame
	 * 
	 * @param pointable
	 *            the pointable you want the acceleration of
	 * @return a PVector containing the acceleration of the tip of the passed
	 *         pointable
	 */
	 ...
	 * 
	 * @return ArrayList<Pointable> an arraylist containing all currently
	 *         tracked pointables
	 */
	...
	/**
	 * returns a pointable by its number. look up to the equivalent methods for
	 * hand/finger for documentation
	 * 
	...
	public Pointable getPointable(int pointableNr) {
		...
		if (!getPointableList().isEmpty()) {
			this.lastDetectedPointable.put(pointableNr,
					getPointableList().get(pointableNr));
		}
		// returnPointable = lastDetectedPointable.get(pointableNr);
		int downCounter = 0;
		while (returnPointable == null) {
			returnPointable = this.lastDetectedPointable.get(pointableNr
					- downCounter);
			downCounter++;
		}
	...
	public Date getTimestamp(Frame frame) {
		Date date = null;
		Set<Entry<Date, Frame>> lastFramesInclDates = this.lastFramesInclProperTimestamps
				.entrySet();
